/* 
  util.c - assorted utility functions
...
inline double
roundoff(double x)		// nearbyint() replacement, with the exception that the result contains a non-zero fractional part
{
  if (x > 0)
    return x + 0.5;
  else
...
inline int32_t
smallprimes(int32_t x)		// returns 1 if x is only made of these small primes
{
  int32_t i, p[2] = { 2, 3 };
...
inline int32_t
nextsprime(int32_t x)		// returns the next integer only made of small primes
{
...
rand_u32()
{
  ...
  return rand();
#elif RAND_MAX == 32767
  return ((rand() % 256) << 24) | ((rand() % 256) << 16) | ((rand() %
							     256) << 8) |
    (rand() % 256);
#else
  fprintf(stderr,
	  "Unhandled RAND_MAX value : %d\nPlease signal this error to the developer.",
	  RAND_MAX);
  return rand();
#endif
}
...
inline uint16_t
fread_le_short(FILE * file)	// read from file a 16-bit integer in little endian
{
...
inline uint32_t
fread_le_word(FILE * file)	// read from file a 32-bit integer in little endian
{
...
inline void
fwrite_le_short(uint16_t s, FILE * file)	// write to file a 16-bit integer in little endian
{
  uint8_t byte;
...
inline void
fwrite_le_word(uint32_t w, FILE * file)	// write to file a 32-bit integer in little endian
{
  uint8_t byte;
...
int32_t
str_isnumber(char *string)	// returns 1 is string is a valid float number, 0 otherwise
{
  int32_t i;
